...
def cnn2(input,size_channel,is_training):
    ...
    return out4
def dcnn(input,size_channel,size_m,is_training,layer_type = 0):
    if(layer_type == 0):
    ...
    if(layer_type == 1):
        ...
        with tf.variable_scope('cnn12'):
            out12 = cnn1(out11,128,is_training)
        with tf.variable_scope('cnn22'):
            out22 = cnn2(out12,512,is_training)
            out22 = up_conv(out22,512)
            #out22 = tf.image.resize_images(out22,(size_m*4,size_m*4),method=0)
            out22 = out22 + tf.concat([out11,out11,out11,out11],axis = 3)
        with tf.variable_scope('cnn21'):
    ...
    if(layer_type == 2):
        with tf.variable_scope('cnn11'):
            out11 = cnn1(input,size_channel,is_training)
        with tf.variable_scope('cnn21'):
            out21 = cnn2(out11,512,is_training)
            out21 = up_conv(out21,512)
            #out21 = tf.image.resize_images(out22,(size_m*8,size_m*8),method=0)
        with tf.variable_scope('cnn3'):
            out = conv_relu(out21,size_channel,3,is_training)
            output = out + input
    return output
def module(input, is_training=True, out_channels=1,kstage=1,size_m = 32,layer_type = 0):
    ...
    with tf.variable_scope("denoise_cnn") as scope:
	    ...
	    delta3_list.append(delta3)
            with tf.variable_scope("", reuse=(False or stage>0)):
                x_out = dcnn(x_in, out_channels, size_m, is_training,layer_type)
            x_in = tf.multiply(delta3, input) + tf.multiply(delta2, x_out)
            out.append(x_out)
    return out,delta1,delta2_list,delta3_list
...
class denoiser(object):
    def __init__(self,sess,config,input_c_dim=1,sigma=25,batch_size=128,kstage=1):
        self.sess = sess
        self.sigma = sigma
        ...
        self.Y_list,self.delta1,self.delta2_list,self.delta3_list = module(self.X, is_training=self.is_training,out_channels=self.input_c_dim,kstage=self.kstage,size_m = self.size_m,layer_type = config.dcnn_layer)
        ...
        self.update_delta3 = []
        for i in range(0,self.kstage):
            if(i == 0):
                self.update_delta3.append(tf.assign(self.delta3_list[i],self.delta1))
            else:
                self.update_delta3.append(tf.assign(self.delta3_list[i],self.delta3_list[i-1]))
        self.train_delta_group = []
        with tf.control_dependencies(update_ops):
            for i in range(0,self.kstage):
                train_list = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='denoise_cnn/d20_%d'%i and 'denoise_cnn/d30_%d'%i)
                self.train_delta_group.append(optimizer.minimize(self.loss_group[i+1], var_list = train_list))
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(start_step, numBatch):
                ...
                if len(pnsr_list) > max_stat:
                    if pnsr_max < pnsr_list[-max_stat] + 0.01 and pnsr_max > pnsr_list[-max_stat] - 0.01:
                        if index < len(lrs)-1:
                            index += 1
                        else:
                            index = 0
                            ...
                            if(stage <= self.kstage):
                                if(config.finetune == 1):
                                    self.sess.run(self.update_delta3[stage-1])
                                    print('finetune delta is given')
                                if(config.finetune == 2):
                                    for i in range(2000):
                                        _ , loss = self.sess.run([self.train_delta_group[stage-1], self.loss_group[stage]],
								                       feed_dict={self.Y_: batch_images, self.lr: lrs[index],self.is_training: True, self.size_m: 6})
                                    for i in range(2000):
                                        _ , loss = self.sess.run([self.train_delta_group[stage-1], self.loss_group[stage]],
								                       feed_dict={self.Y_: batch_images, self.lr: lrs[index+1],self.is_training: True, self.size_m: 6})
                                    print('finetune delta is completed , loss = %.2f'%loss)
                            if(config.pretrain == 1 and stage > config.kstar):
                                stage = self.kstage
                        pnsr_list = []
                        if stage > self.kstage:
                ...
                if(np.mod(batch_id + 1 , 10) == 0):		#print every 10 batch
                    sys.stdout.write("\rStage: [%d] Epoch: [%2d] [%4d/%4d] time: %d,lr: %1.0e, loss: %.2f, psnr: %.2f, Max_psnr: %.2f, Max_hist_psnr: %.2f."
