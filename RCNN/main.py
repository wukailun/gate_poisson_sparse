...
parser.add_argument('--stay_epoch', dest ='stay_epoch',type=int, default=10,help='The epoch number of stay')
...
def main(_):
    ...
    if args.use_gpu:
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            model = denoiser(sess, sigma=args.sigma,kstage = args.kstage,batch_size=args.batch_size)
            if args.phase == 'train':
